...
public class CitationHistogram extends Configured implements Tool {
	...
	@Override
	public int run(String[] path) throws Exception {
		Configuration conf = getConf();
		JobConf job = new JobConf(conf, CitationHistogram.class);
		Path input = new Path(path[0]);
		Path output = new Path(path[1]);
	...
	public static void main(String[] args) throws Exception{
		...
		int res = ToolRunner.run(new Configuration(), new CitationHistogram(),new String[] {inputPath, outputPath});
		System.exit(res);
	}
}
